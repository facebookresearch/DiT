...
class HuggingfaceImageNetDataset(Dataset):
    def __init__(self, token, transform=None, target_transform=None, res=256):
        self.hf_dataset = load_dataset("imagenet-1k", use_auth_token=token)["train"]
        self.token = token
        self.id2class = {0: 'tench, Tinca tinca',
    ...
    def get_item(self, idx):
        image, prompt = self.hf_dataset[idx]["image"], self.id2class[self.hf_dataset[idx]["label"]]

        image = image.convert("RGB")

        prompt = prompt.split(",")[0].lower()
...
class DummyDataset(Dataset):
    def __init__(self, transform=None, target_transform=None, res=256):
